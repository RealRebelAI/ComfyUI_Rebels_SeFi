...
class SEFIDualTimestepEmbeddings(nn.Module):
    ...
    def forward(self, timestep_sem: Tensor, timestep_tex: Tensor) -> Tensor:
        # Ensure inputs are in a valid standard floating precision for the time projection calculations
        target_dtype = next(self.semantic_embedder.parameters()).dtype
        if hasattr(torch, "float8_e4m3fn") and target_dtype in [torch.float8_e4m3fn, torch.float8_e5m2]:
            target_dtype = torch.float32

        sem_proj = self.time_proj(timestep_sem.to(dtype=target_dtype))
        tex_proj = self.time_proj(timestep_tex.to(dtype=target_dtype))
        
        sem_emb = self.semantic_embedder(sem_proj.to(dtype=target_dtype))
        tex_emb = self.texture_embedder(tex_proj.to(dtype=target_dtype))
...
class Flux2SEFITransformer2DModel(nn.Module):
    ...
    def forward(
        ...
        _probe = self.backbone.x_embedder.weight
        model_device = _probe.device
        model_dtype = _probe.dtype
        if hasattr(torch, 'float8_e4m3fn') and model_dtype in (getattr(torch,'float8_e4m3fn',None), getattr(torch,'float8_e5m2',None)):
        ...
        # --- FP8 TIMESTEP ARITHMETIC PATCH ---
        # PyTorch lacks a native CUDA kernel to do element-wise scalar math (* 1000) on Float8 tensors.
        # If the model backbone is quantized down to FP8 variants, fallback to standard float32 for the timesteps.
        is_fp8 = False
        if hasattr(torch, "float8_e4m3fn") and model_dtype == torch.float8_e4m3fn:
            is_fp8 = True
        if hasattr(torch, "float8_e5m2") and model_dtype == torch.float8_e5m2:
            is_fp8 = True

        time_dtype = torch.float32 if is_fp8 else model_dtype

        timestep_sem = timestep_sem.to(device=model_device, dtype=time_dtype)
        timestep_tex = timestep_tex.to(device=model_device, dtype=time_dtype)
        # -------------------------------------
        ...
        # Do the multiplication in the safe math precision, then align the output embedding with model expectations
        temb = self.dual_time_embed(timestep_sem * 1000, timestep_tex * 1000).to(dtype=model_dtype)
        ...
        image_rotary_emb = self.backbone.pos_embed(img_ids.to(model_device))
        text_rotary_emb = self.backbone.pos_embed(txt_ids.to(model_device))
        
        # --- LOW VRAM DEVICE MISMATCH PATCH ---
        concat_rotary_emb = (
            torch.cat([
                text_rotary_emb[0].to(device=model_device, dtype=model_dtype), 
                image_rotary_emb[0].to(device=model_device, dtype=model_dtype)
            ], dim=0),
            torch.cat([
                text_rotary_emb[1].to(device=model_device, dtype=model_dtype), 
                image_rotary_emb[1].to(device=model_device, dtype=model_dtype)
            ], dim=0),
        )
        # --------------------------------------
