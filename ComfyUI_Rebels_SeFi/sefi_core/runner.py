...
class SEFIInferenceRunner:
    ...
    def generate_batch(
        ...
        *,
        ...
        if not torch.isfinite(texture_latents).all():
            raise RuntimeError(
                '[Rebels SeFi] latents went NaN/Inf during sampling (before decode). '
                'Sampler-side numerical blowup - note delta_t/alpha/cfg/steps used.')
        print(f'[Rebels SeFi] latents ok before decode: std={texture_latents.float().std().item():.4f}')
...
class SEFIRunnerDirect(SEFIInferenceRunner):
    """Runner built from prebuilt components (no sefi_config.yaml required).

    Added for the ComfyUI node pack: everything the sampling loop and decode
    path touch is set directly; generate_batch() is inherited unchanged.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # NOTE: intentionally does NOT call super().__init__()
        from diffusers.pipelines.flux2.image_processor import Flux2ImageProcessor

        self.device = torch.device(device)
        self.component_dtype = torch.bfloat16
        self.weight_dtype = weight_dtype

        self.autoguidance_enabled = False
        self.autoguidance_transformer = None
        self.autoguidance_text_encoder = None
        self.autoguidance_reuse_main_text_encoder = True
        self.guidance_interval_sigma_lo = None
        self.guidance_interval_sigma_hi = None
        self.guidance_interval_enabled = False
        self.debug_assert_schedule = False

        self.transformer = transformer
        self.text_encoder = text_encoder
        self.texture_codec = texture_codec
        self.noise_scheduler = noise_scheduler
        self.pipeline_cls = pipeline_cls
        self.semantic_channels = int(semantic_channels)
        self.texture_channels = int(texture_channels)
        self.total_channels = int(semantic_channels + texture_channels)

        self.timestep_shift_alpha = float(timestep_shift_alpha)
        if self.timestep_shift_alpha <= 0:
            raise ValueError("timestep_shift_alpha must be > 0.")
        self.delta_t = float(delta_t)
        if not (0.0 <= self.delta_t <= 1.0):
            raise ValueError("delta_t must be in [0, 1].")
        self.delta_t_min = 0.0
