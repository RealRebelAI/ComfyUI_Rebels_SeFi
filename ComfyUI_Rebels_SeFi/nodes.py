...
class RebelsSeFiLoader:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "delta_t": ("FLOAT", {"default": 0.1, "min": 0.0, "max": 1.0, "step": 0.005,
                                      "tooltip": "Semantic lead. A sefi_config.yaml next to the model file overrides this."}),
    ...
    def load(self, model_name, text_encoder, vae, weight_dtype, text_encoder_device, blocks_on_gpu, unload_encoder_after_encode, delta_t, timestep_shift_alpha):
        ...
        te = LazySeFiEncoder(_build_te, unload_after_encode=bool(unload_encoder_after_encode))
        ...
        resolved_delta = float(derived["delta_t_sidecar"]) if derived["delta_t_sidecar"] is not None else float(delta_t)
    ...
    def sample(self, sefi_pipe, prompt, steps, guidance_scale, width, height, seed):
        ...
        if turbo:
            ...
            if resolved_guidance != 1.0:
                print(f"[Rebels SeFi] Turbo needs guidance 1.0; overriding {resolved_guidance}")
        ...
        def _on_step(i, total):
            if i == 0 and sefi_pipe.get("unload_encoder") and hasattr(runner.text_encoder, "free"):
                runner.text_encoder.free()  # both prompts encoded by now; drop the massive load before loop starts
            now = _time.time()
