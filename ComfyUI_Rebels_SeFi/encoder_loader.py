...
class SeFiQwenEncoderFromFile(nn.Module):
    ...
    def __init__(
        ...
        # --- SEFI FP8 HEAD WIRING FIX ---
        # Qwen ties the lm_head to the embed_tokens. If the FP8 merge stripped 
        # the head to save VRAM, we wire it back up using the embeddings.
        if "lm_head.weight" not in state and "model.embed_tokens.weight" in state:
            state["lm_head.weight"] = state["model.embed_tokens.weight"]
            print("[Rebels SeFi] Info: Wired missing lm_head.weight to embed_tokens.")
        # --------------------------------
...
class LazySeFiEncoder(torch.nn.Module):
    ...
    def encode(self, captions, dtype=None):
        ...
        val = (out[0].detach(), out[1].detach())
        self._cache[key] = val
        
        # Keep cache small to avoid creeping VRAM usage
